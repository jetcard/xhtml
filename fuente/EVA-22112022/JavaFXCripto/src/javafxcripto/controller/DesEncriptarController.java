 ...
 * @author Jyoverar
 */
public class DesEncriptarController extends AppController implements Initializable {
    ...
    private void start() throws IOException {
        ...
        if (tarTexto.getText().isEmpty()) {
            Alert alert = new Alert(Alert.AlertType.INFORMATION, "Delete " + "Escribir el texto a desencriptar", ButtonType.OK);
            alert.showAndWait();
            return;
        }
        ...
        for (String letra : rowTexto) {
            ...
            if (letra.codePointAt(0) != 32) {// no es espacio
                ...
                int valA = 1; //valorLlaveA(letra);// capturando el valor de la letra permitidas
                int valB = valorLlaveB(datosC.get(contLgtC)); // capturando el valor de las letras ordenadas

                int valBBusq = valorLlaveB(letra);

                int valPintar = 0;
                //
                if (valB > valBBusq) {
                    valPintar = lgtB - (valB) + (valBBusq + 1);
                    
                } else {
                    valPintar = valBBusq - valB +1;
                }
                ...
                System.out.println("         valPintar --> " + valPintar + " -> " + datosA.get(valPintar));
                textoEncrip = textoEncrip + datosA.get(valPintar);

                reg = buscarRegla(datosA.get(valPintar));
